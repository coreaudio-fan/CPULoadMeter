...
**CPULoadMeter** is a macOS app that graphs the load on each CPU core over time, drawn by hand in SwiftUI, in a window and in the menu bar. Version 1 (2026-09-22) built the window with its header, period control, and one graph per core; the second iteration (2026-09-24) put the live graph in the menu bar extra, on a monitor of its own with two settings of its own. Deferred, per Design.md §8: activation from the extra's menu (D19) and the app icon (D15). `Design.md` is the specification: its body is the design as decided, and its appendices hold every decision's reasoning and the evidence behind it. See `~/.claude/CLAUDE.md` for the global style guide.
...
- `App/Core/` is the pure core plus the two kernel readers. Nothing here carries an isolation annotation, and nothing needs one: the App target uses the project's default isolation, `nonisolated` (Design.md D21), and these are `Sendable` values and pure functions with no actor to belong to. Isolation is declared where it exists — the views and `CPULoadMeterApp` are main-actor through SwiftUI's `View` and `App` protocols, and the monitor will declare `@MainActor` — never assumed for a whole target and opted out of.
  ...
  - The value types of Design.md §4.1, each holding its invariant by construction and every operation returning a new value: `TickDelta` (the only place a counter is subtracted — in 32 bits with wrapping arithmetic, then widened; deltas add), `CPULoad` (made only from a `TickDelta`, or `zero`), `UsagePercentages` (cumulative rounding, always summing to 100), `LoadHistory` (always exactly `stepCount` loads, oldest first), `SamplingPeriod` (1…60 through failable initializers only), `HistoryLength` (30…120 likewise; `stepCount(at:)` is the whole periods it holds, never fewer than one), and `MonitorState` (`advanced(with:)` is the entire per-sample logic; a CPU-count change, the first sample included, is a baseline only). `SamplingPeriod` and `HistoryLength` share the `WholeSeconds` protocol — seconds, presets, parse from text — which the one control is generic over. The monitor is their client; the test bundle reaches them through the app's debug dylib.
- `App/LoadMonitor.swift` is the sampler: `@Observable @MainActor`, owning one loop `Task`, the period, the `MonitorState`, and the last error, and persisting nothing. Its reader and its sleep are passed in (`@MainActor` function types; defaults are the real reader and `Task.sleep` on the continuous clock), which is what makes every behavior testable without a clock: the tests drive it with `ScriptedTicks` and `ScriptedSleep`. Its `deinit` is `isolated` — a plain `deinit` cannot cancel the main-actor `loop`. There are two: the `App` creates both in `init` and owns them as `@State`. The window's comes from the stored period and the stored window width; `MainWindowContent` resumes it on appear and suspends it on disappear, so its sampling runs only while the window is shown and a reopened window starts fresh (D23). The menu bar's comes from its own stored period and history length; `MenuBarGraphLabel` resumes it when the extra's label appears and nothing suspends it (D25). Each carries a `name` that prefixes its log lines — `[window]`, `[menu bar]`.
- `App/Views/` holds the SwiftUI views. `LoadStackView` is **one `Canvas` for every CPU** (Design.md D22): one row per CPU, the hairlines drawn on the row boundaries, and per row the drawing rule of §5.5 in `appendLines(of:to:in:lineWidth:)` — a line per step rising from the row's bottom edge, newest at the right, the line's center half a width left of a whole point; `lineWidth` is a parameter, and the menu-bar graph will draw with the same function. One canvas because SwiftUI's cost is layout, not strokes (Findings). It carries no accessibility elements — `accessibilityChildren` was tried and VoiceOver never read them, attaching only to the header's two controls (D22). It reports the step count its width holds through `onGeometryChange` — only after it has appeared, because SwiftUI lays the window out once at a default 900-pt width before placing it and that report would truncate a windowless history (Findings below). **`MenuBarGraph`** is the menu bar's canvas: every CPU's history side by side, CPU 0 leftmost, drawn with the same `appendLines`; from the left, a 3 pt divider slot, a 1 pt gap, the graph, a gap, and so on, ending with a slot, so the first and last slots are endcaps; slots at 25% black, 16 pt tall, in black and clear only because it is shown as a template image. The user chose the divider style from a sampler after 1 pt dividers read as samples (Design.md D.6); do not thin them. **`MenuBarGraphLabel`** is the extra's label and the one place the menu bar's monitor is read: on every sample it renders `MenuBarGraph` through `ImageRenderer` at the display scale and shows `Image(decorative:scale:).renderingMode(.template)`, which the system colors for the menu bar (Design.md D24, B.18). A `Canvas` as the label draws nothing — SwiftUI realizes the label as the status item button's image (Findings below) — so do not "simplify" it back to the canvas. It also applies the two stored settings to the monitor when they change. `HeaderView` is the two lines of §2.6 — name and core count with `SecondsControl` beside them, the usage line in `top`'s wording below (`CPU usage: —` before the first load, `CPU usage unavailable` while samples fail) — and `SecondsControl<Value: WholeSeconds>` is the combo box of §5.9 composed from a `TextField` and a borderless `Menu`, used for the window's period and for the menu bar's period and history length: the draft commits on Return and on focus loss through `committedValue(from:current:)`, the reject-and-revert rule as one pure function. `SettingsView` holds the launch checkbox and the two menu bar controls, owns one `@FocusState` per field, and clears both on a click on the form. `MainView` owns the field's `@FocusState` and clears it on a click on the graphs or the header's text — nothing else in the window can take focus, so without that a click elsewhere never ends editing. Switching apps or windows is deliberately *not* a commit: the draft waits, as in every Mac app (Design.md D.6, P5). `MainView` is the layout of §5.3. `MenuBarExtraMenu`'s *Show CPULoadMeter* calls `openWindow` and nothing else: with another app active it does **not** bring the app to the foreground, a known limitation of version 1 that Design.md D19 and §8 record and defer. The design is pure SwiftUI — no `import AppKit`, no app delegate, no `NSViewRepresentable` — and any AppKit call needs a decision recorded in the spec; see the Findings below for the two that were tried here and rolled back.
...
- `App/CPULoadMeterApp.swift` declares the scenes. Their order is load-bearing: `Window` first, `Settings`, `MenuBarExtra` last, and the extra always inserted, because the extra's presence is what lets the app outlive its window and gives the window its Window-menu entry (Design.md B.1, D.2). The extra's label is `MenuBarGraphLabel` with the menu bar's monitor; its menu is `MenuBarExtraMenu`, unchanged.
- `App/Diagnostics.swift` is the `os.Logger`, subsystem `coreaudio-fan.CPULoadMeter`, category `monitor`. Its messages are debug level and present in every build; read them with `/usr/bin/log stream --debug --predicate 'subsystem == "coreaudio-fan.CPULoadMeter"'`. Every line but the launch line starts with the monitor's name in brackets.
- `App/DefaultsKey.swift` names the six `UserDefaults` keys. The app and its views read and write defaults; the core, the readers, and the monitors touch none.
...
The suite uses **swift-testing** — `import Testing`, `struct` suites, `@Test` functions, `#expect`/`try #require`. 76 tests in 16 suites as of this writing.
...
### The view hierarchy was the cost (2026-09-24)

At a 397×792 window, 1 s period, 24 CPUs: one `Canvas` per CPU cost the app 1.5–1.6% of a core; one canvas for all CPUs, 0.9–1.0%, over three runs each. The `sample` profile put the app's own frames in SwiftUI layout (`LayoutEngineBox.sizeThatFits`, `StackLayout.placeChildren`), with path building at 6 samples in 47,000. Logging was measured innocent (a listener attached, or the line removed: no change), the Debug build was barely heavier, and neither the accessibility children (since removed: VoiceOver never read them) nor the separator style cost anything measurable. WindowServer's share could not be told from noise with `top` — its window-closed readings swung by 20 points between runs. Lesson for the menu-bar graph: draw everything in one canvas; never a view per element that updates every sample. Design.md D22 and D.6.
...
### The extra's label is the status item's image (2026-09-24)

A `Canvas` given as a `MenuBarExtra`'s label draws nothing: its body was re-evaluated once a second, its drawing closure never ran, and from inside the process the `NSStatusBarButton` had no image and the item was 16 pt wide. An `Image` label becomes the button's image and is replaced on every body evaluation — a new image object each tick — and `.renderingMode(.template)` sets the image's `isTemplate`. So the graph is rendered to a `CGImage` with `ImageRenderer` on every sample; the shipped item is 1,579 × 22 pt with a 1,563 × 16 pt template image (1,479 and 1,463 before the dividers were widened). It costs the app 1.5–1.6% of a core with the window closed (three runs; 0.0 at rest before). Neither the mechanism nor the width is documented; Design.md D24, B.18, and D.6 have the probe and the numbers. The shell cannot see the item — `CGWindowListCopyWindowInfo` lists nothing and `screencapture` refuses — so what the tinted item looks like is a check by eye.
