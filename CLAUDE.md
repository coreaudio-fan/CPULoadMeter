...
**CPULoadMeter** is a macOS app that graphs the load on each CPU core over time, drawn by hand in SwiftUI, with a Menu Bar Extra that will ultimately carry the graph in the menu bar. Version 1 as specified is complete (2026-09-22): the window with its header, period control, and one graph per core; the static extra; sampling for the life of the process. Deferred, per Design.md §8: activation from the extra's menu (D19), the app icon (D15), and the live graph in the menu bar (P10), which is the next iteration. `Design.md` is the specification: its body is the design as decided, and its appendices hold every decision's reasoning and the evidence behind it. See `~/.claude/CLAUDE.md` for the global style guide.
