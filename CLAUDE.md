...
- `App/Views/` holds the SwiftUI views. `MainView` is a placeholder until the header and the graphs land. `MenuBarExtraMenu` holds **the app's one AppKit call**, after `openWindow`: `NSRunningApplication.current.activate(from: NSWorkspace.shared.frontmostApplication ?? .current, options: [])`, the cooperative form of an activation request, naming the frontmost app as the one handing over. SwiftUI's environment has no action that activates an app; without one *Show CPULoadMeter* did not bring the app to the foreground when another app was frontmost, and the bare `NSApplication.shared.activate()` changed nothing — the system declines it, while it granted the cooperative form in every probe cycle (Design.md D19, D.6). Do not "simplify" it back to `activate()`. The design is otherwise pure SwiftUI — no app delegate, no `NSViewRepresentable` — and any further AppKit call needs the same kind of decision, recorded in the spec.
...
- ***Show CPULoadMeter* did not bring the app to the foreground** with another app frontmost. SwiftUI has no activation action; one AppKit call was admitted (see `MenuBarExtraMenu` above). The first form tried, `NSApplication.activate()`, changed nothing: a probe from inside the app with Finder in front showed the system declines it every time, along with the deprecated `activate(ignoringOtherApps:)`, while `NSRunningApplication.activate(from:options:)` naming the frontmost app was granted every time and made the window key without any window call. `makeKeyAndOrderFront` alone only got the window to second place — a window cannot pass another app's key window (Apple, `orderFrontRegardless()`). Activation stays a request the system may decline, so the fix awaits its re-check from the menu.
