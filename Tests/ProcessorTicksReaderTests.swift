...
struct ProcessorTicksReaderTests {
	...
	//	"Every reply buffer is freed" is not expressible in the type system, so this runtime check stands in for
	//	it. The kernel maps a fresh 16 KB page per call, so leaking would cost about 64 MB over these calls; the
	//	bound is a quarter of that, leaving room for whatever else the host process does meanwhile. Signed
	//	arithmetic, because the footprint can shrink. Design.md, section 6 and D.4.
