...
The button calls `openWindow(id: "main")`, which opens the window or brings an open one to the front, and then `NSApplication.shared.activate()`, which asks the system to make the app active. That is the app's one AppKit call. SwiftUI's environment offers no action that activates an app, and with `openWindow` alone, selecting the item while another app was frontmost did not bring this app to the foreground. Activation is a request the system may decline; it is made in response to the user's click on the menu, and whether the system honors it there is a bring-up check (§7, P9).
...
| **P9** | *Failed 2026-09-21 (D.6): selecting the item while another app was frontmost did not bring this app to the foreground. Resolved by the one AppKit call (D19), which is a request the system may decline; to be re-checked with the click.* A window opened from the extra's menu comes forward while another app is active. | §2.4, §5.2 |
...
| **D19** | *Show CPULoadMeter* did not bring the app to the foreground (P9). Activate through AppKit, or not? | One AppKit call, `NSApplication.shared.activate()`, after `openWindow`. The pure-SwiftUI principle admits a single recorded exception where SwiftUI has no route; the alternatives were a URL-scheme round trip through Launch Services, or accepting the behavior. | You | §1, §5.1, §5.2 | B.1, D.6 |
...
- **Bringing up the scaffold** (2026-09-21). The first checks against the running app, in D.6. The automated ones (P2, P8, P12) and most of the hand checks (P1, P3, P13) held; the stored window size needed the hidden title bar's safe-area inset added; P9 failed and one AppKit call was admitted (D19); the extra proved removable and its removal was accepted (D20).
...
| **P9** the extra's item brings the window forward | **Failed.** You. | With another app frontmost, *Show CPULoadMeter* did not bring the app to the foreground. Resolved by D19; the re-check is pending. |
