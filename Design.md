...
- The app places an item in the system menu bar, shown as the live graph: every CPU's load history side by side, the first CPU leftmost, with a divider between neighbors. Each CPU's graph is drawn by the rules of §2.8, one point per step, and is as many steps wide as the menu bar's history length holds whole periods of the menu bar's update period: 60 s of history at a 1 s period is 60 steps; 60 s at 7 s is 8, the partial period dropped; a length shorter than the period still gives one step. The graph is 16 pt tall. With 24 CPUs at the defaults the image is 1,463 pt wide, which is accepted.
...
- The graph is handed to the system as a template image, black and clear, which the system colors for the menu bar it is in, light or dark, and for the item's selected state. The plot is opaque; the dividers are black at 40% opacity.
...
| Menu bar graph | a template image of black and clear, colored by the system for the menu bar; the dividers black at 40% opacity |
...
- **`App-*.xcconfig`** — modelled on SourceTools' app: `SUPPORTED_PLATFORMS = macosx`, `MACOSX_DEPLOYMENT_TARGET = 26.0`, `RUN_DOCUMENTATION_COMPILER = NO`, the sandbox entitlement and hardened runtime, `INFOPLIST_KEY_NSHumanReadableCopyright`, bundle ID `coreaudio-fan.CPULoadMeter`, and category `public.app-category.utilities`. No `LSUIElement`: this is a regular app that also has a menu bar extra. Unlike SourceTools' app, no `SWIFT_DEFAULT_ACTOR_ISOLATION` override: the project baseline, `nonisolated`, applies to every target (D21).
...
`internal`, `Sendable` value types in `App/Core/`. They carry no isolation annotation and need none: the project's default isolation is `nonisolated` for every target (D21), and a pure value has no actor to belong to. Isolation is declared where it exists — the views and the `App` are main-actor through SwiftUI's `View` and `App` protocols, and the monitor declares `@MainActor` (§4.2).
...
`MenuBarGraph` draws with the same function in one `Canvas` of its own: CPU *i*'s history in the rectangle at *x = i × (steps + 1)*, `steps` wide and the full 16 pt tall, and a 1 pt divider filling the point before every graph but the first, black at 40% opacity over an opaque black plot, since the image is a template (§2.4). Its width is *CPUs × steps + (CPUs − 1)*: 1,463 pt for 24 CPUs at 60 steps.
...
- **The menu bar item's width.** At 24 CPUs and the defaults it is 1,479 pt, about half of a 3,072-pt display's menu bar. One point per step was chosen knowing that (D27); a narrower step, or a cap on the width, stays available.
...
| **D23** | Sample while the window is hidden? | No (2026-09-24). Sampling runs only while the main window is shown; closing it stops the loop and drops the history, and reopening starts over from a fresh baseline, as at launch. The step count is kept. The extra's live graph, when it comes, revisits this. | You | §2.2, §2.10, §4.2, §5.1, §5.8 | B.9 |
...
| **The menu bar graph** (2026-09-24): it is 16 pt tall; the dividers are 1 pt wide, black at 40% opacity; a history length shorter than the period gives one step, not zero; the menu bar's sampling starts when the label first appears; the label shows the `cpu` symbol when there is nothing to render. | §2.4, §2.13, §5.2 |
...
- **Actor isolation needs a decision the framework made implicitly.** The app target defaults to `MainActor` isolation, as SourceTools' does; with the pure core in that target, its value types would inherit it. They are marked `nonisolated` so they stay usable off the main actor (P7 may move sampling there) **[R]** — to be settled against the compiler at P12. Settled, then reversed (D21, 2026-09-22): the annotations compiled and the guard fired as expected **[O]**, but you judged the `MainActor` default itself the wrong choice — a crutch that puts a whole target in one isolation domain so that a simple app need not think about synchronization, and a poor design for anything else — and pointed out that the project baseline in every sibling is `nonisolated`; SourceTools' app-target override was the odd one out, inherited here with its stated reason, "mirroring Apple's app template", and without weighing it. The override is gone, every `nonisolated` in the app with it, and isolation is declared only where it exists.
...
**The item's width.** At 24 CPUs and the defaults the image is 1,463 pt wide and the item 1,479 **[O]**, about half of a 3,072-pt display's menu bar. You chose one point per step knowing that (D27); a narrower step, or a cap on the width, stays available (§8).
...
| The `nonisolated` guard | **Fired as a build error**, not a test; superseded by D21. | With `nonisolated` dropped from `readProcessorTicks()`, the test build failed at `ProcessorTicksReaderTests.swift:19`: "main actor-isolated global function 'readProcessorTicks()' cannot be called from outside of the actor". On 2026-09-22, with `CPULoad` alone stripped, the app build itself failed too (8 errors in `App/Core`, its nonisolated siblings unable to reference `zero` or pass `init` to `map`) and the test build 40 times over, `Equatable` included. Then D21 removed the `MainActor` default: with the App target on the project's `nonisolated` baseline there is no annotation to miss, and the guard has nothing to guard. |
...
| Menus rebuilt on every sample | **Found by hand after PR 4, and fixed.** You. | With PR 4 merged, an open menu showed one set of items for a couple of seconds and then a different set, and sometimes closed with the button still down. A probe writing a line from the `App`'s body showed it evaluated once a second — 8 times in 8 s, intervals 0.98–1.06 s — because the body read `monitor.state` to hand `MainView` its values, and Observation re-evaluates a scene body like a view body; the scenes' re-evaluation rebuilds the menus. With the reads moved into `MainWindowContent`, the window's root view, the same probe counted the app's body once in 8 s and the window content's nine times: the re-evaluation is confined to the window. Whether an open menu now stays put is the hand check. |
...
| The renderer and short lines | Observation from the pixel tests, 2026-09-22. | `ImageRenderer`'s output of a `Canvas` carries no antialiasing: every pixel is either the label color's full alpha, 216 of 255 here, or zero. Lines three device pixels and taller are exactly the geometry at 1x and 2x, at 1 pt and 0.5 pt; lines one or two pixels tall are drawn one pixel taller, at both scales. Neither is documented. The tests use loads whose lines are all three pixels or taller, and what the two shortest look like on screen is part of P4. |
