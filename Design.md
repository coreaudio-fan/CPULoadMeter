...
2. **Pure SwiftUI.** There are no AppKit bridges: no app delegate and no `NSViewRepresentable`. Where SwiftUI has no route at all, a single AppKit call is admitted and recorded where it is made. There is one: activating the app from the menu bar extra (§5.2).
...
- Its menu has two items: **Show CPULoadMeter**, which shows the main window or brings it forward and makes the app active, and **Settings…**.
...
There is no app delegate. The one `import AppKit` is in `MenuBarExtraMenu`, for the one call §5.2 describes.
...
`MenuBarExtraMenu` has two items: a button titled *Show CPULoadMeter*, and a `SettingsLink`.

The button calls `openWindow(id: "main")`, which opens the window or brings an open one to the front, and then asks the system to activate the app with `NSRunningApplication.current.activate(from: NSWorkspace.shared.frontmostApplication ?? .current, options: [])` — the cooperative form of the request, naming the frontmost app as the one handing activation over. That is the app's one AppKit call, and its form matters: SwiftUI's environment offers no action that activates an app; with `openWindow` alone, selecting the item while another app was frontmost did not bring this app to the foreground; and the bare `NSApplication.activate()` changed nothing either, because the system declines it when no interaction it recognizes precedes it. Activation stays a request the system may decline, so the call's result is deliberately ignored: nothing further can be done with a refusal. Whether the system grants the request from the menu is a bring-up check (§7, P9).
...
| **P9** | *Failed 2026-09-21 (D.6): selecting the item while another app was frontmost did not bring this app to the foreground, and the bare `NSApplication.activate()` added for it changed nothing. Replaced by the cooperative form of the request (D19), granted in every probe cycle; to be re-checked with the click.* A window opened from the extra's menu comes forward while another app is active. | §2.4, §5.2 |
...
| **D19** | *Show CPULoadMeter* did not bring the app to the foreground (P9). Activate through AppKit, or not? | One AppKit call after `openWindow`: the cooperative activation request, `NSRunningApplication.activate(from:options:)` naming the frontmost app. The pure-SwiftUI principle admits a single recorded exception where SwiftUI has no route; the alternatives were a URL-scheme round trip through Launch Services, or accepting the behavior. The bare `NSApplication.activate()` was tried first and changed nothing; a probe showed the system declines it and grants the cooperative form. | You; the form by experiment | §1, §5.1, §5.2 | B.1, D.6 |
...
- **Bringing up the scaffold** (2026-09-21). The first checks against the running app, in D.6. The automated ones (P2, P8, P12) and most of the hand checks (P1, P3, P13) held; the stored window size needed the hidden title bar's safe-area inset added; P9 failed and one AppKit call was admitted (D19) — the bare `NSApplication.activate()` first, which changed nothing, then the cooperative form a probe showed the system grants; the extra proved removable and its removal was accepted (D20).
