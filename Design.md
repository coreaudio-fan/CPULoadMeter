...
- **Commit** happens on Return (`onSubmit`), when the field loses focus (`@FocusState`), and when the window stops being the key window (`controlActiveState`). The draft is parsed through `SamplingPeriod.init?(text:)`: either a period comes out, or the draft reverts to the current period.
- **Nothing else in the window can take focus**, so a click elsewhere would leave the field editing. The main view owns the focus state and clears it when the graphs or the header's text are clicked, which is what makes a click elsewhere commit; switching to another window or app does not move SwiftUI's focus at all, which is why the key-window change commits on its own.
